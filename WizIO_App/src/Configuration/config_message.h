...
#define REMOTE_CLEAR            0x10
#define REMOTE_LOCATE           0x20
...
typedef struct __WIZnet_Clear
{
    uint8_t dst_mac_address[6];
    uint8_t set_pw_len;
    uint8_t set_pw[16];
}__attribute__((packed)) WIZnet_Clear;

typedef struct __WIZnet_Clear_Reply
{
    uint8_t src_mac_address[6];
}__attribute__((packed)) WIZnet_Clear_Reply;

typedef struct __WIZnet_Locate
{
    uint8_t dst_mac_address[6];
    uint8_t set_pw_len;
    uint8_t set_pw[16];
}__attribute__((packed)) WIZnet_Locate;

typedef struct __WIZnet_Locate_Reply
{
    uint8_t src_mac_address[6];
}__attribute__((packed)) WIZnet_Locate_Reply;
