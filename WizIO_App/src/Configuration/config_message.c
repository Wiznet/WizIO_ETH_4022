...
static void ProcessClear(uint8_t sock, Remote_Info *remote_info, WIZnet_Header *wiznet_header)
{
    uint16_t len;
    uint8_t buffer[sizeof(WIZnet_Header) + sizeof(WIZnet_Clear_Reply)];
    uint32_t ptr;
    WIZnet_Clear clear;
    WIZnet_Clear_Reply reply;
    WIZIOPacket *s2e_packet = GetWIZIOPacketPointer();
    uint8_t ip[4];
    uint16_t port;
    uint8_t i;
    ...
    recvfrom(sock, (uint8_t *) &clear, sizeof(WIZnet_Factory_Reset), ip, &port);

    if ((wiznet_header->valid & 0x80)) Decrypt((wiznet_header->valid & 0x7F), (uint8_t *) &clear, len);

    if (memcmp(clear.dst_mac_address, s2e_packet->network_info_common.mac, 6)) return;

    if (strcmp((char *) clear.set_pw, (char *) s2e_packet->options.pw_setting)) return;

    for (i = 0; i < DI_NUM; i++) {
        PWM_Cmd(gk_DigitalInPwmChannel[i], DISABLE);
        PWM_Cmd(gk_DigitalInPwmChannel[i], ENABLE);
    }
    ...
    if ((wiznet_header->valid & 0x80)) Encrypt((wiznet_header->valid & 0x7F), (uint8_t *) &reply, sizeof(WIZnet_Clear_Reply));

    ptr = 0;
    memcpy(buffer, wiznet_header, sizeof(WIZnet_Header));
    ptr += sizeof(WIZnet_Header);
    memcpy(buffer + ptr, (void *) &reply, sizeof(WIZnet_Clear_Reply));
    ptr += sizeof(WIZnet_Clear_Reply);
...
static void ProcessLocate(uint8_t sock, Remote_Info *remote_info, WIZnet_Header *wiznet_header)
{
    uint16_t len;
    uint8_t buffer[sizeof(WIZnet_Header) + sizeof(WIZnet_Locate_Reply)];
    uint32_t ptr;
    WIZnet_Locate find;
    WIZnet_Locate_Reply reply;
    WIZIOPacket *s2e_packet = GetWIZIOPacketPointer();
    uint8_t ip[4];
    uint16_t port;
    ...
    recvfrom(sock, (uint8_t *) &find, sizeof(WIZnet_Clear), ip, &port);

    if ((wiznet_header->valid & 0x80)) Decrypt((wiznet_header->valid & 0x7F), (uint8_t *) &find, len);

    if (memcmp(find.dst_mac_address, s2e_packet->network_info_common.mac, 6)) return;

    if (strcmp((char *) find.set_pw, (char *) s2e_packet->options.pw_setting)) return;

    g_buzzer_locate_flag = 1;

    // reply
    wiznet_header->length = sizeof(WIZnet_Locate_Reply);
    wiznet_header->op_code[1] = WIZNET_REPLY;

    memcpy(reply.src_mac_address, s2e_packet->network_info_common.mac, 6);

    if ((wiznet_header->valid & 0x80)) Encrypt((wiznet_header->valid & 0x7F), (uint8_t *) &reply, sizeof(WIZnet_Locate_Reply));

    ptr = 0;
    memcpy(buffer, wiznet_header, sizeof(WIZnet_Header));
    ptr += sizeof(WIZnet_Header);
    memcpy(buffer + ptr, (void *) &reply, sizeof(WIZnet_Locate_Reply));
    ptr += sizeof(WIZnet_Locate_Reply);
            ...
            if (len >= sizeof(WIZnet_Header)) {
                ...
                switch (wiznet_header.op_code[0] & 0xF0)
                {
                    ...
                    case FACTORY_RESET:
                        ...
                        break;
                    case REMOTE_CLEAR:
                        ProcessClear(sock, &remote_info, &wiznet_header);
                        break;
                    case REMOTE_LOCATE:
                        ProcessLocate(sock, &remote_info, &wiznet_header);
                        break;
                    default:
                        break;
                }
