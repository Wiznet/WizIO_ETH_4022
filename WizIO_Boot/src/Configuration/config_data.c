...
void CheckMacAddress(void)
{
    ...
    if (g_s2e_packet.network_info_common.mac[0] != 0x00 || g_s2e_packet.network_info_common.mac[1] != 0x08 || g_s2e_packet.network_info_common.mac[2] != 0xDC) {
        do {
            myprintf("Input MAC ? ");
#if BOARD == WIZWIKI_W7500
            buf[0] = UartGetc(UART1);
#else
            buf[0] = S_UartGetc();
#endif
            if (buf[0] == 'S') {
        ...
        for (i = 0, j = 0; i <= 6; i++, j += 2)
            g_s2e_packet.network_info_common.mac[i] = GetHex(buf[j], buf[j + 1]);
